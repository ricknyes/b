...
#logo
logo = pyfiglet.figlet_format('*      bidu      *')
print(X+logo)
print('  ')
print(A+'═'*60)
print('  ')
...
try:
    ...
    bot_token = "6770027926:AAHfZ4-ujAukmacK3-HyVeXyxNxJgoNXYjM"
    DEVLOO = 6449831912
    MAX_ACCOUNTS = 100
    id_bot = 6770027926
    user_bot = "snnsnsn_bot"
    ...
    async with bot.conversation(event.chat_id) as conv:
        ...
        if answer.text == 'نعم':
            ...
            for file in os.listdir():
                if file not in ['run.py', 'bidukalb.py', 'bidu.json', '__pycache__', 'bidu-telethon-cl.py', 'bot.session']:
                    os.remove(file)
        elif answer.text == 'لا':
...
async def callback(event):
    await event.edit("**• قـم بأرسال المطاليب التالية : تعديل بيدووو**", buttons=[Button.inline("• رجــوع • ", 'list1')])
    async with bot.conversation(event.sender_id) as conv:
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('رشق مشاهدات ⟐', 'akbo')],
         ...
         [Button.inline('༺ 🔱 بيدو 🔱 𝐁𝐎𝐓 ༻', 'button0')]
        ]
    ...
    async with bot.conversation(event.sender_id) as conv:
        ...
        try: 
            ...
            if not client.is_connected():
                ...
                return
            if not await client.is_user_authoribidud():
